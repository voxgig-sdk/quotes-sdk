...
boru-driven command-line client **and** interactive REPL for the Quotes
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/quotes-cli)
make build
...
./quotes-cli load 1 owner            # {id:1} shorthand
./quotes-cli load '{id:1}' owner       # explicit match map

# 5. Override the API base URL for a single call
QUOTES_BASE=https://api.example.com ./quotes-cli load 1 owner
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/quotes-cli
   ```
...
```sh
./quotes-cli load 1 owner          # scalar shorthand for {id:1}
./quotes-cli load '{id:1}' owner     # explicit match map
```
...
Run with no arguments to open a REPL (prompt `quotes>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `owner`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/quotes-cli`. |
...
(`./quotes-cli <expr>`) and interactively (the REPL), and expressions compose the
