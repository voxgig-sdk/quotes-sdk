...
```php
try {
    // load() returns the ENTITY — call data_get() for the Owner record (throws on error).
    $owner = $client->Owner()->load();
    print_r($owner->data_get());
} catch (\Throwable $err) {
...
The main class (`quotes_sdk.php`) exports the SDK class
