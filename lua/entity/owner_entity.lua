...
local OwnerEntity = {}
OwnerEntity.__index = OwnerEntity


function OwnerEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, OwnerEntity)
  self._name = "owner"
  self._client = client
...
function OwnerEntity:get_name()
  return self._name
end


function OwnerEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return OwnerEntity.new(self._client, opts)
end
...
function OwnerEntity:mark_deleted()
  self._deleted = true
end


function OwnerEntity:deleted()
  return true == self._deleted
end


function OwnerEntity:data_set(args)
  if args ~= nil then
...
function OwnerEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function OwnerEntity:match_set(args)
  if args ~= nil then
...
function OwnerEntity:match_get()
...
function OwnerEntity:stream(action, args, callopts)
...
---@param reqmatch OwnerLoadMatch
---@param ctrl? table
---@return Owner
---@return string? err
function OwnerEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function OwnerEntity:_run_op(ctx, post_done)
  local utility = self._utility
