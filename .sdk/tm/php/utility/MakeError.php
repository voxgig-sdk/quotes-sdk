...
class QuotesMakeError
{
    public static function call(?QuotesContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new QuotesContext([], null);
        }
        $op = $ctx->op ?? new QuotesOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new QuotesResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof QuotesError) ? $err->msg : (string)$err;
        ...
        $sdk_err = new QuotesError('', $msg, $ctx);
