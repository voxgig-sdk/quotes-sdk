...
//    only - a typed QuotesContext there would make every ctx
...
final class QuotesOmni
{
    ...
    public static function ctx_from_map($ctxmap, $client, $utility)
    {
        ...
        $ctx = new QuotesContext($clean, null);
    ...
    public static function err_from_map($m): ?QuotesError
    {
        if (!is_array($m)) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new QuotesError(is_string($code) ? $code : '', $msg);
    }
