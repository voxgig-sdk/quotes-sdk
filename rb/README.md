...
The Ruby SDK for the Quotes API — an entity-oriented client using idiomatic Ruby conventions.

The SDK exposes the API as capitalised, semantic **Entities** — for example `client.Owner` — with named operations (`list`/`load`) instead of raw URL paths and query strings. Working with resources and verbs keeps call sites self-describing and reduces cognitive load.
...
```ruby
client = QuotesSDK.test(testopts, sdkopts)
```
...
raise a `QuotesError` (a `StandardError` subclass), so wrap
...
The main module (`Quotes_sdk`) exports the SDK class
