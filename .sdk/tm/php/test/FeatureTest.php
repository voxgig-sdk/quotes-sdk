...
class FtCtrl extends QuotesControl
{
...
class FtClient
{
    ...
    private QuotesUtility $utility;

    public function __construct(QuotesUtility $utility, array $options)
    {
    ...
    public function get_utility(): QuotesUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (QuotesContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (QuotesContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = QuotesConfig::make_feature($name);
    ...
    private static function buildUrl(QuotesSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof QuotesError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new QuotesResponse($fetched);
        }
    ...
    private function fail(QuotesContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (QuotesContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
