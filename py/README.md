...
The Python SDK for the Quotes API. Provides an entity-oriented interface following Pythonic conventions.
...
```python
result, err = client.Owner(None).load({"id": "example_id"}, None)
if err:
    raise Exception(err)
print(result)
```
...
result, err = client.Quotes(None).load(
    {"id": "test01"}, None
)
