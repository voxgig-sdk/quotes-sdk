...
class QuotesSDK
{
    ...
    public function __construct(array $options = [])
    {
        ...
        $feature_opts = QuotesHelpers::to_map(Struct::getprop($this->options, "feature"));
        if ($feature_opts) {
            $featureorder = Struct::getpath($this->options, "__derived__.featureorder");
            if (is_array($featureorder)) {
                foreach ($featureorder as $fname) {
                    $fopts = QuotesHelpers::to_map($feature_opts[$fname] ?? null);
                    if ($fopts && isset($fopts["active"]) && $fopts["active"] === true) {
                        ...
                        if (!QuotesFeatures::has_feature($fname)) {
                        ...
                        ($utility->feature_add)($this->_rootctx, QuotesFeatures::make_feature($fname));
                    }
                }
            }
        }
    ...
    {
        return QuotesUtility::copy($this->_utility);
    }
    ...
    public function prepare(array $fetchargs = []): mixed
    {
        ...
        $ctrl = QuotesHelpers::to_map(Struct::getprop($fetchargs, "ctrl")) ?? [];
        ...
        $params = QuotesHelpers::to_map(Struct::getprop($fetchargs, "params")) ?? [];
        $query = QuotesHelpers::to_map(Struct::getprop($fetchargs, "query")) ?? [];
    ...
    private function op_denied(string $op): array
    {
        ...
        return [
            "ok" => false,
            "err" => new QuotesError($op . "_allow",
                "QuotesSDK: " . $op . ": operation not allowed by" .
    ...
    private function raw_request(array $fetchargs = []): mixed
    {
        ...
        if (is_array($fetched)) {
            $status = QuotesHelpers::to_int(Struct::getprop($fetched, "status"));
    ...
    public function graphql(string $query, ?array $variables = null, ?array $ctrl = null): mixed
    {
        ...
        if (is_array($errors) && 0 < count($errors)) {
            ...
            $res["ok"] = false;
            $res["err"] = new QuotesError("graphql_error",
                "QuotesSDK: graphql: " . $msg);
            $res["graphql"] = $errors;
        }
    ...
    // Canonical facade: $client->Owner()->list() / ->load(["id" => ...]).
    // PHP method names are case-insensitive, so lowercase $client->owner()
    // resolves here too.
    public function Owner($data = null)
    {
        require_once __DIR__ . '/entity/owner_entity.php';
        if ($data === null) {
            if ($this->_owner === null) {
                $this->_owner = new OwnerEntity($this, null);
            }
            return $this->_owner;
        }
        return new OwnerEntity($this, $data);
    }
