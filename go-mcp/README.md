...
[MCP](https://modelcontextprotocol.io) server exposing the Quotes SDK as
two agent tools — `quotes_list` and `quotes_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/quotes-mcp)
make build

# 2. Provide credentials via the environment
export QUOTES_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user quotes \
  -- /absolute/path/to/quotes-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./quotes-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// quotes_list: first page of records
{ "entity": "quote" }
{ "entity": "quote", "query": { } }

// quotes_load: one record by id
{ "entity": "owner", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/quotes-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user quotes \
     -- "$PWD"/dist/*/quotes-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `quotes_list` and `quotes_load` tools now appear
   in new sessions. Ask the agent to *"list quote using quotes"*
   and it calls `quotes_list` with `{"entity":"quote"}`.
...
```sh
export QUOTES_APIKEY=sk_live_xxx            # API key
export QUOTES_BASE=https://api.example.com  # optional: override the API base URL
```
...
| Tool | Args | Returns |
|------|------|---------|
| `quotes_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `quotes_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `QUOTES_APIKEY` | API key sent with every request. |
| `QUOTES_BASE` | Optional override of the API base URL. |
...
```sh
./quotes-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"quotes_load","arguments":{"entity":"owner","query":{"id":1}}}}'
```
