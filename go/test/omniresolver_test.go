...
import (
	...
	omni "github.com/voxgig-sdk/quotes-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.QuotesError); is {
				out := map[string]any{
					"name":    "QuotesError",
...
func omniCtx(arg any, client *sdk.QuotesSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.QuotesSDK); is {
			client = live
