...
class TestOwnerEntity:
    ...
    def test_should_create_instance(self):
        ...
        ent = testsdk.Owner(None)
        assert ent is not None

    def test_should_run_basic_flow(self):
        setup = _owner_basic_setup(None)
        # Per-op sdk-test-control.json skip — basic test exercises a flow with
        # multiple ops; skipping any one skips the whole flow (steps depend
        # on each other).
        _live = setup.get("live", False)
        for _op in ["load"]:
            _skip, _reason = runner.is_control_skipped("entityOp", "owner." + _op, "live" if _live else "unit")
            if _skip:
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set QUOTES_TEST_OWNER_ENTID JSON to run live")
        client = setup["client"]

        # Bootstrap entity data from existing test data.
        owner_ref01_data_raw = vs.items(helpers.to_map(
            vs.getpath(setup["data"], "existing.owner")))
        owner_ref01_data = None
        if len(owner_ref01_data_raw) > 0:
...
def _owner_basic_setup(extra):
    runner.load_env_local()

    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/owner/OwnerTestData.json")
    ...
    idmap_resolved = helpers.to_map(
        env.get("QUOTES_TEST_OWNER_ENTID"))
    ...
    if env.get("QUOTES_TEST_LIVE") == "TRUE":
        ...
        client = QuotesSDK(helpers.to_map(merged_opts))

    _live = env.get("QUOTES_TEST_LIVE") == "TRUE"
    return {
        "client": client,
        "data": entity_data,
        "idmap": idmap_resolved,
        "env": env,
        "explain": env.get("QUOTES_TEST_EXPLAIN") == "TRUE",
        "live": _live,
