...
class QuoteEntityTest extends TestCase
{
    ...
    public function test_stream(): void
    {
        ...
        $base = QuotesSDK::test($seed, null);
        $seen = iterator_to_array($base->Quote(null)->stream("list", null, null), false);
        $this->assertCount(3, $seen);

        // Inbound: streaming active -> yields each item from the feature.
        $cfg = QuotesConfig::shared_config();
        if (isset($cfg["feature"]) && is_array($cfg["feature"]) && isset($cfg["feature"]["streaming"])) {
            $sdk = QuotesSDK::test($seed, ["feature" => ["streaming" => ["active" => true]]]);
            $got = [];
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set QUOTES_TEST_QUOTE_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
...
function quote_basic_setup($extra)
{
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["quote01", "quote02", "quote03", "random01", "random02", "random03"] as $k) {
    ...
    $entid_env_raw = getenv("QUOTES_TEST_QUOTE_ENTID");
