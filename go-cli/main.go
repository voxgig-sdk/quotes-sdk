// Command Quotes-cli is a CLI + REPL for the Quotes SDK,
...
const entitiesHelp = "owner quote"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      QUOTES_APIKEY (api key), QUOTES_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: QUOTES_APIKEY carries the API key and
	// QUOTES_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("QUOTES_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("QUOTES_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewQuotesSDK(opts)
