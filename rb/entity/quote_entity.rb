...
class QuoteEntity
  ...
  def data_set(args)
    if args
      @_data = QuotesHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  def match_set(args)
    if args
      @_match = QuotesHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  # @return [Quote, Hash] the loaded Quote; raises QuotesError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = QuotesHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
  ...
  # @param ctrl [Object, nil] optional per-call control
  # @return [Array<Quote>, Array] the matching Quote items; raises QuotesError on failure
