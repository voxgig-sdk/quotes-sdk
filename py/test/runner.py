...
class QuotesTestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            QuotesTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = QuotesTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = QuotesTestRunner.getenv("QUOTES_TEST_LIVE")
        override = QuotesTestRunner.getenv("QUOTES_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = QuotesTestRunner.getenv(key)
        ...
        explain = QuotesTestRunner.getenv("QUOTES_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["QUOTES_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if QuotesTestRunner._test_control is not None:
            return QuotesTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                QuotesTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            QuotesTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = QuotesTestRunner.load_test_control()
...
def load_env_local():
    QuotesTestRunner.load_env_local()


def env_override(m):
    return QuotesTestRunner.env_override(m)


def entity_data(v):
    return QuotesTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return QuotesTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return QuotesTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return QuotesTestRunner.load_test_control()


def live_client_options():
    return QuotesTestRunner.live_client_options()


def live_delay_ms():
    return QuotesTestRunner.live_delay_ms()
