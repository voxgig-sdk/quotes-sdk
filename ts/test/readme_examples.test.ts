...
//     (\`new QuotesSDK(...)\` / \`QuotesSDK.test(...)\` become
//     \`QuotesSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const TEST_SEED = {"entity":{"owner":{"test01":{"id":"test01"}},"quote":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new QuotesSDK(...)\` and any
// \`QuotesSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+QuotesSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/QuotesSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
