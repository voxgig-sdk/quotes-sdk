...
MCP server exposing the Quotes SDK as tools, built on the
...
| Tool | Args | Returns |
|------|------|---------|
| `quotes_list` | `entity`, optional `query` map | First page of records as JSON |
| `quotes_load` | `entity`, `query` (e.g. `{id:N}`) | Single record as JSON |
...
After install, restart Claude Code; the `quotes_list` and
`quotes_load` tools become available in new sessions.
