...
{
    private static function utility(): QuotesUtility
    {
        return new QuotesUtility();
    }
    ...
    private static function ctx(array $over = []): QuotesContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new QuotesContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new QuotesOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof QuotesError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new QuotesOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = QuotesMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new QuotesOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = QuotesMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new QuotesOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = QuotesMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new QuotesSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = QuotesMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            ...
            'result' => new QuotesResult([]),
        ]);
        [, $err] = QuotesMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new QuotesSpec(['step' => 's']),
            'response' => new QuotesResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new QuotesResult([]),
        ]);
        [, $err] = QuotesMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new QuotesSpec(['step' => 's']),
            'response' => new QuotesResponse(self::resp(200, ['v' => 1])),
            'result' => new QuotesResult([]),
        ]);
        [, $err] = QuotesMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new QuotesSpec(['step' => 's']),
            'response' => new QuotesResponse(self::resp(200, ['v' => 2])),
            'result' => new QuotesResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        QuotesMakeResponse::call($ctx);
    ...
    {
        $preset = new QuotesResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new QuotesSpec([]),
            'response' => new QuotesResponse([]),
            'result' => new QuotesResult([]),
        ...
        [$got, $err] = QuotesMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new QuotesResult([])]);
        [, $err] = QuotesMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new QuotesSpec([]), 'result' => null]);
        [, $err] = QuotesMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new QuotesSpec(['step' => 's']),
            'result' => new QuotesResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new QuotesOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = QuotesMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new QuotesSpec(['step' => 's']),
            'result' => new QuotesResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new QuotesOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = QuotesMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new QuotesResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new QuotesSpec([]),
            'result' => new QuotesResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = QuotesMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = QuotesMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new QuotesError('boom', 'boom');
        $utility->fetcher = function (QuotesContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new QuotesSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = QuotesMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (QuotesContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = QuotesMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(QuotesResponse::class, $response);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (QuotesContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new QuotesSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = QuotesMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new QuotesResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new QuotesSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = QuotesMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = QuotesMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = QuotesMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new QuotesResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, QuotesDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new QuotesResult(['ok' => false])]);
        $this->expectException(QuotesError::class);
        QuotesDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new QuotesResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', QuotesMakeError::call($ctx, null));
    }
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new QuotesBaseFeature();
        $b = new QuotesBaseFeature();
        QuotesFeatureAdd::call($ctx, $a);
        QuotesFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): QuotesBaseFeature
    {
        $f = new QuotesBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        QuotesFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): QuotesContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new QuotesSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = QuotesPrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new QuotesResponse(['headers' => null]),
            'result' => new QuotesResult([]),
        ]);
        QuotesResultHeaders::call($ctx);
