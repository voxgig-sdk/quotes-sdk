...
The Lua SDK for the Quotes API. Provides an entity-oriented interface using Lua conventions.
...
```lua
local result, err = client:Owner(nil):load({ id = "example_id" }, nil)
...
local result, err = client:Quotes(nil):load(
  { id = "test01" }, nil
)
